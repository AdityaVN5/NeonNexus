...
async function seed() {
  ...
  try {
    ...
    for (let i = 0; i < NUM_USERS; i += USER_BATCH) {
        ...
        if ((i + USER_BATCH) % 100000 === 0) console.log(`Inserted ${i + USER_BATCH} users...`);
    }
    ...
    for (let i = 0; i < NUM_SESSIONS; i += SESSION_BATCH) {
        ...
        if ((i + SESSION_BATCH) % 500000 === 0) console.log(`Inserted ${i + SESSION_BATCH} sessions...`);
    }
